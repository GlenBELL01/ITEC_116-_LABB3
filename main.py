...
############################################################################################################
############################################################################################################
##      LAB ACTIVITY 4: SOLUTION
##      Endpoint: /detailed_post/{userID}
##      Method: GET
##      - Given the userID, show all the posts of that specific user and all comments per each post.
############################################################################################################
from fastapi import FastAPI
import requests
import json
...
def fetch_user_posts_with_comments(user_id: int):
    ...
    for single_post in all_posts:
        ...
        if single_post['userId'] == user_id:
            ...
            response_data["posts_with_comments"].append(post_detail)
